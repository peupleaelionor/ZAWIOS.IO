      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-[var(--text2)] mt-1 text-sm">
            Suivez vos signaux créés et vos votes
          </p>
        </div>
      ...
      {predictions.length > 0 ? (
        ...
        <div className="surface rounded-2xl p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-[var(--text)] mb-2">
            Aucun signal {activeTab === 'created' ? 'créé' : activeTab === 'voted' ? 'voté' : 'résolu'}
          </h3>
          <p className="text-sm text-[var(--text2)] mb-4 max-w-xs mx-auto">
            {activeTab === 'created'
              ? 'Créez votre premier signal pour commencer à construire votre historique.'
              : activeTab === 'voted'
                ? 'Parcourez les signaux et enregistrez votre premier vote.'
                : 'Les signaux résolus apparaîtront ici une fois les résultats confirmés.'}
          </p>
          <Link href={activeTab === 'created' ? '/signals/create' : '/signals'}>
            <Button size="sm" variant="outline">
              {activeTab === 'created' ? 'Créer un signal' : 'Explorer les signaux'}
            </Button>
          </Link>
        </div>
      )}
